   ...
   IN OUT	PMESSAGE_REPLY_DATA pReplyMessage)
{
	...
	PPROCESS_INFO 	pSendMessage = (PPROCESS_INFO)messageSend;

	ULONG callbackType = pSendMessage->MessageType;
	...
	__try
	{
		...
		case FILTER_SEND_THREAD_HANDLE_INFO:
		 ...
		 wprintf( L"Id#%d UserName:%ws\\%ws\nProcessId:%d ThreadId:%d callbackType:%0x ImageFileName:%ws\n"
			 ,pSendMessage->MessageId,domainName,userName,pSendMessage->ProcessId,pSendMessage->ThreadId,callbackType,pSendMessage->ImageFileName);


		if(pReplyMessage && pSendMessage->MessageType == FILTER_SEND_PROCESS_CREATION_INFO )
		{
